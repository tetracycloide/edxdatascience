...
# Code: Simulated data with xsubj=d+ejsubj
J <- 6
N <- 2000
d <- .021
p <- (d+1)/2
X <- d + rnorm(J, 0, 2*sqrt(p*(1-p)/N))
# Code: Simulated data with xsubi,j=d+esubi,j
I <- 5
J <- 6
N <- 2000
d <- .021
...
# Code: Simulated data with xdubi,j = d + hsubi + esubi,j
I <- 5
J <- 6
N <- 2000
d <- .021
...
# Note that sigma now includes an estimate of the variability due to general bias of .025.
mu <- 0
tau <- 0.035
